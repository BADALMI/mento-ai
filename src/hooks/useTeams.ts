...
export function useTeams(authUser: User | null) {
  ...
  const joinTeam = async (roomCode: string) => {
    ...
    try {
      ...
      // Use the RPC to find team by room code, bypassing RLS
      const { data: rpcData, error: rpcError } = await supabase
        .rpc('find_team_by_room_code', { code: cleanRoomCode });

      if (rpcError) {
        console.error('Error finding team:', rpcError);
        throw new Error('Error searching for team. Please try again.');
      }

      const teamData = rpcData && rpcData.length > 0 ? {
        id: rpcData[0].team_id,
        name: rpcData[0].team_name,
        room_code: rpcData[0].team_room_code,
        created_by: rpcData[0].team_created_by
      } : null;
